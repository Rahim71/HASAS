...
void clSpectDirDipole2::Calculate (int iScaling, stpSpectDir2RN spRemoveNoise)
{
    long lSensorCntr;
    long lSpectCntr;
    GDT fMagnitude;
    ...
    for (lSensorCntr = 0l; lSensorCntr < 2l; lSensorCntr++)
    {
        ...
        if (spRemoveNoise->iType != SDD2_BNER_NONE)
        {
...
clSpectDirDipole2::clSpectDirDipole2 (GDT fSensorSpacing, GDT fSoundSpeed,
    double dSampleRate, long lFiltSize, int iFilterType, long lWindowSize, 
    GDT fIntTimeReq, bool bEnableDebug)
{
    long lSensorCntr;
    long lTwosPower;
    GDT fWindowTime;
...
void clSpectDirDipole2::PutData (const GDT *fpInputData, long lSampleCount, 
...
bool clSpectDirDipole2::GetResults (GDT *fpLevelResults, GDT *fpDirResults, 
    GDT fLowFreqLimit, int iScaling, stpSpectDir2RN spRemoveNoise)
{
    ...
    for (lSensorCntr = 0l; lSensorCntr < 2l; lSensorCntr++)
    {
        if (lOldData <= 0l)
        {
            if (!Decimator[lSensorCntr].Get(fpProcBuf[lSensorCntr], lFFTSize))
                return false;
        }
        else
        {
