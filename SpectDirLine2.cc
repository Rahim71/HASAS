...
void clSpectDirLine2::Calculate (int iScaling, stpSpectDir2RN spRemoveNoise)
{
    long lSensorCntr;
    long lSpectCntr;
    GDT fMagnitude;
    ...
    for (lSensorCntr = 0l; lSensorCntr < lSensorCount; lSensorCntr++)
    {
        ...
        if (spRemoveNoise->iType != SDL2_BNER_NONE)
        {
    ...
    for (lSensorCntr = 0l; lSensorCntr < (lSensorCount - 1); lSensorCntr++)
    {
        ...
        for (lSpectCntr = lMinBin; lSpectCntr < lMaxBin; lSpectCntr++)
        {
            ...
            fpLevRes[lSpectCntr] += fMagnitude;
            fpDirRes[lSpectCntr] *= GetDirection(fFrequency, fPhaseDiff);
        }
    }
    fScaleExp = (GDT) 1.0 / (GDT) (lSensorCount - 1);
    for (lSpectCntr = lMinBin; lSpectCntr < lMaxBin; lSpectCntr++)
    {
        fpDirRes[lSpectCntr] = pow(fpDirRes[lSpectCntr], fScaleExp);
    }
}


clSpectDirLine2::clSpectDirLine2 (long lSensors, GDT fSensorSpacing, 
    GDT fSoundSpeed, double dSampleRate, long lFiltSize, int iFilterType, 
    long lWindowSize, GDT fIntTimeReq, bool bEnableDebug)
{
    long lSensorCntr;
    long lTwosPower;
    GDT fWindowTime;
    ...
    fpWinFunc = (GDT *) WinFuncBuf.Size(lFFTSize * sizeof(GDT));
    DSP.WinExactBlackman(fpWinFunc, lFFTSize);
    fpLevRes = (GDT *) LevResBuf.Size(lSpectSize * sizeof(GDT));
    fpDirRes = (GDT *) DirResBuf.Size(lSpectSize * sizeof(GDT));
...
void clSpectDirLine2::PutData (const GDT *fpInputData, long lSampleCount, 
...
bool clSpectDirLine2::GetResults (GDT *fpLevelResults, GDT *fpDirResults, 
    GDT fLowFreqLimit, int iScaling, stpSpectDir2RN spRemoveNoise)
{
    long lSensorCntr;
    ...
    DSP.Zero(fpLevRes, lSpectSize);
    DSP.Zero(fpDirRes, lSpectSize);
    Calculate(iScaling, spRemoveNoise);
    if (fpLevelResults != NULL)
        DSP.Copy(fpLevelResults, fpLevRes, lSpectSize);
    if (fpDirResults != NULL)
        DSP.Copy(fpDirResults, fpDirRes, lSpectSize);
    return true;
}
